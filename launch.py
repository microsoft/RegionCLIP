...
def main():
    ...
    if "OMPI_COMM_WORLD_SIZE" in os.environ:
        if args.nnodes != int(os.environ["OMPI_COMM_WORLD_SIZE"]):
            args.nnodes = int(os.environ["OMPI_COMM_WORLD_SIZE"])
    else:
        assert args.nnodes > 0, "number of nodes should be larger than 0!!!"
    ...
    master_addr = current_env['MASTER_ADDR']
    master_port = current_env['MASTER_PORT']
    ...
    cmd.append("--num-machines {}".format(args.nnodes))
    cmd.append("--machine-rank {}".format(args.node_rank))
    cmd.append("--dist-url {}".format("tcp://{}:{}".format(master_addr,str(master_port))))
    #cmd.append("--num-gpus={}".format(args.nproc_per_node))
