...
class FocalNet(Backbone):
    ...
    def forward(self, x):
        ...
        if len(self.out_indices) == 0:
            outs["stage5"] = x_out.view(-1, H, W, self.num_features[i]).permute(0, 3, 1, 2).contiguous()
...
def build_clip_focal_backbone(cfg, input_shape):
    """
    ...
    return FocalNet(            
        ...
        focal_windows=spec_vision['FOCAL_WINDOWS'],   
        use_conv_embed=spec_vision['USE_CONV_EMBED'],    
        use_postln=spec_vision['USE_POSTLN'],       
        use_postln_in_modulation=spec_vision['USE_POSTLN_IN_MODULATION'], 
        scaling_modulator=spec_vision['SCALING_MODULATOR'],
        use_layerscale=spec_vision['USE_LAYERSCALE'], 
        out_indices=(0, 1, 2, 3),
        use_checkpoint=spec_vision['USE_CHECKPOINT'],
    )